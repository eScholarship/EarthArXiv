...
def register_ezid_doi(identifier):
    from utils import setting_handler

    domain = identifier.article.journal.domain
    pingback_url = urlencode({'pingback': 'http://{0}{1}'.format(domain, reverse(_ezid_pingback'))})

    test_url = '{0}&{1}'.format(EZID_TEST_URL, pingback_url)
    live_url = '{0}?{1}'.format(EZID_LIVE_URL, pingback_url)

    use_ezid = setting_handler.get_setting('Identifiers', 'use_ezid',
                                               identifier.article.journal).processed_value

    if not use_ezid:
        logger.info("[DOI] Not using EZID DOIs on this journal. Aborting registration.")
        return 'EZID Disabled', 'Disabled'

    test_mode = setting_handler.get_setting('Identifiers', _ezid_test', identifier.article.journal).processed_value

    if test_mode:
        util_models.LogEntry.add_entry('Submission', "EZID DOI registration running in test mode", 'Info',
                                       target=identifier.article)
    else:
        util_models.LogEntry.add_entry('Submission', "EZID DOI registration running in live mode", 'Info',
                                       target=identifier.article)

    return send_ezid_deposit(test_url if test_mode else live_url, identifier)


def register_ezid_component(article, xml, supp_file):
    from utils import setting_handler

    test_url = EZID_TEST_URL
    live_url = EZID_LIVE_URL
    ...
    test_mode = setting_handler.get_setting(
            'Identifiers', _ezid_test', article.journal
    ).processed_value or settings.DEBUG

    server = test_url if test_mode else live_url

    if test_mode:
        util_models.LogEntry.add_entry('Submission', "EZID DOI component registration running in test mode", 'Info',
                                       target=article)
    else:
        util_models.LogEntry.add_entry('Submission', "EZID DOI component registration running in live mode", 'Info',
                                       target=article)
    ...
    response = requests.post(server, data=xml.encode('utf-8'),
                             auth=(setting_handler.get_setting('Identifiers',
                                                               _ezid_username',
                                                               article.journal).processed_value,
                                   setting_handler.get_setting('Identifiers', _ezid_password',
                                                               article.journal).processed_value),
                             headers={"Content-Type": "application/vnd_ezid.deposit+xml"})
...
def send_ezid_deposit(server, identifier):
    ...
    template_context = {
        ...
        'depositor_name': setting_handler.get_setting('Identifiers', _ezid_name',
                                                      identifier.article.journal).processed_value,
        'depositor_email': setting_handler.get_setting('Identifiers', _ezid_email',
                                                       identifier.article.journal).processed_value,
        'registrant': setting_handler.get_setting('Identifiers', _ezid_registrant',
                                                  identifier.article.journal).processed_value,
    ...
    template = 'common/identifiers_ezid.xml'
_ezid_template = render_to_string(template, template_context)
    logger.debug_ezid_template)

    util_models.LogEntry.add_entry('Submission', "Sending request to {1}: {0}".format_ezid_template, server),
                                   'Info',
                                   target=identifier.article)

    logger.debug("[EZID:DEPOSIT:%s] Sending to %s" % (identifier.article.id, server))
    response = requests.post(server, data_ezid_template.encode('utf-8'),
                             auth=(setting_handler.get_setting('Identifiers',
                                                               _ezid_username',
                                                               identifier.article.journal).processed_value,
                                   setting_handler.get_setting('Identifiers', _ezid_password',
                                                               identifier.article.journal).processed_value),
                             headers={"Content-Type": "application/vnd_ezid.deposit+xml"})
    logger.debug("[EZID:DEPOSIT:%s] Response code %s" % (identifier.article.id, response.status_code))
    if response.status_code != 200:
        util_models.LogEntry.add_entry('Error',
                                       "Error depositing: {0}. {1}".format(response.status_code, response.text),
                                       'Debug',
...
def create_ezid_doi_identifier(article, doi_suffix=None, suffix_is_whole_doi=False):
    ...
    if not suffix_is_whole_doi:
        doi_prefix = setting_handler.get_setting('Identifiers', _ezid_prefix', article.journal)
...
def generate_ezid_doi_with_pattern(article):
    """
    Creates a_ezid doi utilising a preset pattern.
    ...
    doi_prefix = setting_handler.get_setting('Identifiers', _ezid_prefix', article.journal).value
...
def register_preprint_doi(request,_ezid_enabled, identifier):
    """
    Registers a preprint doi with_ezid, has its own function as preprints dont have things like issues.
    ...
    if not_ezid_enabled:
        ...
        template = 'common/identifiers_ezid.xml'
        ezid_template = render_to_string(template, template_context)

        pdfs = identifier.article.pdfs
        if len(pdfs) > 0:
            template_context['pdf_url'] = identifier.article.pdf_url

        response = requests.post(url, data_ezid_template.encode('utf-8'),
